...
% 参数搜索
n_Bo = 100;
Bo_range = linspace(0.2, 0.3, n_Bo);   % 经过多次拟合选定最终的拟合范围
...
for i = 1:n_Bo
    ...
    theta_range = linspace(pi/2 + 0.05, 0.75*pi, n_grid);
    ...
    for cc = 1:n_grid
        ...
        for tt = 1:n_grid
                theta0 = theta_range(tt);
                    ...
                    % 严格检查液桥半径最小处位置相同
                    if abs(r_sol_min - r_min) > 0.03 || abs(z_sol_min - z_min) > 0.2 || abs(z_bottom - L_norm) > 0.05
                        continue;
                    end
