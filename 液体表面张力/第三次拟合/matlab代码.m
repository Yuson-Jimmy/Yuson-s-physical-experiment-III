...
% 参数搜索
n_Bo = 100;
Bo_range = linspace(0, 0.2, n_Bo);   % 经过多次拟合选定最终的拟合范围
...
for i = 1:n_Bo
    ...
    theta_range = linspace(pi/2 + 0.05, 0.9*pi, n_grid);
    ...
    for cc = 1:n_grid
        ...
        for tt = 1:n_grid
                theta0 = theta_range(tt);
                    ...
                    [~, y_sol] = ode45(odefun, [0 2*L_norm], y0, options);
                    ...
                    % 严格检查液桥半径最小处位置相同
                    if abs(r_sol_min - r_min) > 0.03 || abs(z_sol_min - z_min) > 0.1 || abs(z_bottom - L_norm) > 0.02
                        continue;
                    end
...
if ~isnan(best_Bo)
    ...
    [~, y_best] = ode45(odefun_best, [0 2*L_norm], [1; 0; best_theta0], ...
